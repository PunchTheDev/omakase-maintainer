from omakase_maintainer.state import RERUN_COOLDOWN_S, State
...
def test_open_pr_cap(tmp_path):
    ...
    assert st.admit("hk", "omakase-router")[0]
    st.enqueue("omakase-router#1", "omakase-router", 1, "hk", "alice", "sha")
    st.set_status("omakase-router#1", "running")
    ok, reason = st.admit("hk", "omakase-router")
...
def test_rerun_cooldown(tmp_path):
    ...
    st.touch_miner("hk", "alice")
    st.enqueue("omakase-router#1", "omakase-router", 1, "hk", "alice", "s")
    st.set_status("omakase-router#1", "closed")
    assert not st.admit("hk", "omakase-router")[0]  # just decided → cooldown
...
def test_credibility_decay_bans(tmp_path):
    ...
    assert not st.admit("hk", "omakase-router")[0]
